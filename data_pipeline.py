        ...
        if isinstance(image_dir, list):
            for image_dir_temp in image_dir:
                ...
                for label in label_list:
                    label_dir = image_dir_temp +'/'+ label
...
#             if test_train exists, set the filter for test labels
            ...
            elif self.dataset in ['eurecom', 'eurecom_new']:
...
#                label_train_arr = train_arr[:,1].astype(np.float)
                label_train_arr = self.prepare_targets(train_arr[:,1]).astype(np.float)
                self.image_test_arr = test_arr[:,0]
                label_test_arr = self.prepare_targets(test_arr[:,1]).astype(np.float)
...
#            elif test_train=='test':
    ...
    def prepare_targets(self, y_train):
    	le = LabelEncoder()
    	le.fit(y_train)
...
#                        print(image_depth_path)
                    elif self.dataset == 'eurecom_gen':
#                        image_depth_path = image_path.replace('/RGB/','/depth/')    
                        #gen Depth
                        image_depth_path = image_path.replace('/RGB/','/depth_new/')
#                        image_depth_path = image_depth_path.replace('rgb_','depth_')  
#                        print(image_depth_path)
...
#                        print('depth loaded', image_depth_arr.shape)
...
#                print(image_depth_arr.shape)
                print('datagen failed for {} and {}'.format(image_path))
                break          
